...
test("脚本启动时会清理旧缓存 key 且不会复用旧值", async () => {
    const { result, persistentData } = await runResponseCase({
        url: "https://api.trakt.tv/movies/123",
        body: readFixture("movie-detail.json"),
        persistentData: {
            trakt_zh_cn_cache_v2: createMediaTranslationCache(),
            trakt_comment_translation_cache: createCommentTranslationCache(),
            trakt_current_season_cache: JSON.stringify({ showId: "555", seasonNumber: 3 })
        }
    });

    const payload = JSON.parse(result.body);
    assert.equal(payload.title, "Original Title");
    assert.equal(persistentData.trakt_zh_cn_cache_v2, null);
    assert.equal(persistentData.trakt_comment_translation_cache, null);
    assert.equal(persistentData.trakt_current_season_cache, null);
});
